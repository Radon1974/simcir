...
//Вывод на экран состояния устройства
  ...
  */
  ...
  // зарегистрировать источник постоянного тока
  ...
  // регистрация простого светодиода
  ...
  // регистрировать логические элементы
  ...
  // устарело. не отображается в панели инструментов по умолчанию.
  ...
  // зарегистрировать поворотный энкодер
  ...
  $s.registerDevice('BusOut', function(device) {
    ...
    device.halfPitch = true;
    for (var i = 0; i < numInputs; i += 1) {
      device.addInput();		//добавление входных узлов
    }
    device.addOutput('', 'x' + numInputs);		//добавдение выходного узла
    device.$ui.on('inputValueChange', function() {
      var busValue = [];
      var hotCount = 0;
      for (var i = 0; i < numInputs; i += 1) {
